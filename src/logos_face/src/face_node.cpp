...
const char KEY_INCREASE_ROWS = 's';
const char KEY_DECREASE_ROWS = 'w';
...
public:
    FaceNodeCpp() : nh_("~"), quit_requested_(false) {
        ...
        // Publishers
        ascii_pub_ = nh_.advertise<std_msgs::String>("/face/live_state/ascii", 10);
        notification_led_pub_ = nh_.advertise<std_msgs::Int32MultiArray>("/notification/rgbled", 10);

        // Existing live state publisher
...
private:
    ...
    void initEyeParams() {
        current_left_ = {0,0,1,1,1,0,"#90ee90"};     // LightGreen
        current_right_ = {0,0,1,1,0.5,0,"#90ee90"}; 
    ...
    void renderCallback(const ros::TimerEvent&) {
        ...
        // Publish ASCII
        std_msgs::String ascii_msg;
        ascii_msg.data = ascii;
        ascii_pub_.publish(ascii_msg);
    ...
    void renderEye(cv::Mat &img, int offset_x, const EyeParams &eye) {
        ...
        cv::ellipse(img, cv::Point(center_x, center_y), cv::Size(sx, sy), 0, 0, 360, waveform_color_scalar, 1);
        ...
        cv::line(img, cv::Point(lid_x1, lid_y1), cv::Point(lid_x2, lid_y2), colScalar, 8);
    ...
    void renderWaveform(cv::Mat &img) {
        ...
        if (has_audio) {
            ...
            */
            ...
            // --------------------
            // 2. Render Combined Waveform (Color-Cycling, Single Pixel Width)
            // --------------------
            ...
            for (int i = 0; i < length; ++i) {
                ...
                if (!first_combined_point) {
                    ...
                    cv::line(img, cv::Point(x - 1, prev_y_combined), cv::Point(x, y), color, 2);
                }
                prev_y_combined = y;
        ...
        // --------------------
        // 3. Render Sine Wave (Defined, 2 Pixel Width)
        // --------------------
    ...
    std::string imageToAscii(const cv::Mat &img, int cols, int rows) {
        ...
        // We will scale the image to fit exactly cols x rows-1
        // caca_dither_bitmap expects width, height in terms of canvas cells
        // We'll just use cols and rows directly as the output size
