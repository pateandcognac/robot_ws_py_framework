...
public:
    FaceNodeCpp() : nh_("~"), quit_requested_(false) {
        ...
        mouth_trail_enabled_ = nh_.param<bool>("mouth_trail_enabled", true);
        mouth_trail_half_life_ = nh_.param<double>("mouth_trail_half_life", 0.5);
...
private:
    ...
    void configCallback(logos_face::FaceNodeConfig& config, uint32_t) {
        ...
        if (config.mouth_trail_enabled != mouth_trail_enabled_) {
            mouth_trail_enabled_ = config.mouth_trail_enabled;
            if (!mouth_trail_enabled_ && !mouth_trail_bgr_.empty()) {
                mouth_trail_bgr_.setTo(cv::Scalar(0, 0, 0));
            }
            mouth_trail_last_update_ = ros::Time::now();
        }

        if (std::abs(config.mouth_trail_half_life - mouth_trail_half_life_) > 1e-6) {
            mouth_trail_half_life_ = std::max(0.0, static_cast<double>(config.mouth_trail_half_life));
            mouth_trail_last_update_ = ros::Time::now();
        }
    ...
    void recreateRenderBuffersLocked(int width, int height) {
        ...
        mouth_trail_bgr_.create(render_height_, render_width_, CV_8UC3);
        mouth_trail_bgr_.setTo(cv::Scalar(0, 0, 0));
        mouth_trail_last_update_ = ros::Time::now();
    ...
    void renderWaveform(cv::Mat& img) {
        ...
        if (mouth_trail_enabled_ && mouth_trail_half_life_ > 0.0) {
            updateMouthTrail(img, sine_wave_buffer_, baseline);
        } else {
            if (!mouth_trail_bgr_.empty()) {
                mouth_trail_bgr_.setTo(cv::Scalar(0, 0, 0));
            }
            mouth_trail_last_update_ = ros::Time::now();
            drawSineWave(img, sine_wave_buffer_, baseline);
        }

        drawAudioWaveform(img, baseline);
    }

    void updateMouthTrail(
        cv::Mat& img,
        const std::vector<float>& sine_wave,
        int baseline
    ) {
        if (mouth_trail_bgr_.empty() ||
            mouth_trail_bgr_.rows != img.rows ||
            mouth_trail_bgr_.cols != img.cols) {
            mouth_trail_bgr_.create(img.rows, img.cols, CV_8UC3);
            mouth_trail_bgr_.setTo(cv::Scalar(0, 0, 0));
            mouth_trail_last_update_ = ros::Time::now();
        }
        ...
        if (!mouth_trail_last_update_.isZero()) {
            const double dt = std::max(0.0, (now - mouth_trail_last_update_).toSec());
            const double decay = std::pow(0.5, dt / mouth_trail_half_life_);
            mouth_trail_bgr_.convertTo(mouth_trail_bgr_, -1, decay, 0.0);

            const int trail_black_floor = 4;
            cv::inRange(
                mouth_trail_bgr_,
                cv::Scalar(0, 0, 0),
                cv::Scalar(trail_black_floor, trail_black_floor, trail_black_floor),
                black_mask_
            );
            mouth_trail_bgr_.setTo(cv::Scalar(0, 0, 0), black_mask_);
        }

        mouth_trail_last_update_ = now;

        drawSineWave(mouth_trail_bgr_, sine_wave, baseline);
        cv::add(img, mouth_trail_bgr_, img);
    }
    ...
    void drawSineWave(
        ...
        for (int i = 0; i < static_cast<int>(wave.size()); ++i) {
            const int x = i;
            const int y = static_cast<int>(
                baseline + (wave[i] * static_cast<float>(img.rows) * 0.125f)
            );
    ...
    void drawAudioWaveform(cv::Mat& img, int baseline) {
        const int length = img.cols;
