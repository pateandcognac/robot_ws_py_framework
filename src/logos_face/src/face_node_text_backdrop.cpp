...
private:
    ...
    void ensureRenderGeometryLocked() {
        ...
        const int target_h = std::max(
            min_render_height_,
            static_cast<int>(std::lround(canvas_h * render_px_per_char_y_))
        );
    ...
    void drawMarqueeBackdropLocked(int w, int h) {
        ...
        caca_set_color_ansi(caca_canvas_, backdrop_color_, backdrop_bg_color_);
        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                if (ch >= 0x20 && ch < 0x7f) {
                    caca_put_char(caca_canvas_, x, y, ch);
                }
            }
        }
    }
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "logos_face_text_backdrop");
    FaceNodeCpp node;
    node.run();
    return 0;
}
