...
#include <logos_msgs/SpeechData.h>
...
// +/- = decrease/increase caption pane height
// \ = clear screen
...
const char KEY_INCREASE_CAPTION_PANE = '+';
const char KEY_INCREASE_CAPTION_PANE_ALT = '=';
const char KEY_DECREASE_CAPTION_PANE = '-';
...
public:
    FaceNodeCpp() : nh_("~"), quit_requested_(false) {
        ...
        min_fps_ = 1;
        max_fps_ = 24;
        display_fps_ = std::max(1, nh_.param<int>("display_fps", 30));
        ...
        backdrop_text_topic_ = nh_.param<std::string>("backdrop_text_topic", "/face/text_backdrop");
        backdrop_mode_ = nh_.param<std::string>("backdrop_mode", "terminal");
        backdrop_direction_ = nh_.param<std::string>("backdrop_direction", "left");
        backdrop_scroll_cps_ = std::max(0.0, nh_.param<double>("backdrop_scroll_cps", 8.0));
        backdrop_row_offset_ = std::max(0, nh_.param<int>("backdrop_row_offset", 7));
        backdrop_color_ = colorNameToCaca(nh_.param<std::string>("backdrop_color", "green"));
        backdrop_bg_color_ = colorNameToCaca(nh_.param<std::string>("backdrop_bg_color", "black"));
        backdrop_max_lines_ = std::max(1, nh_.param<int>("backdrop_max_lines", 120));
        ...
        caption_pane_fraction_ = clampDouble(nh_.param<double>("caption_pane_fraction", 0.33), 0.05, 0.80);
        caption_pane_step_ = clampDouble(nh_.param<double>("caption_pane_step", 0.05), 0.01, 0.25);
        caption_font_ = nh_.param<std::string>("caption_font", "thick");
        status_font_ = nh_.param<std::string>("status_font", "computer");
        figlet_font_dir_ = nh_.param<std::string>("figlet_font_dir", findDefaultFigletFontDir());
        status_caption_topic_ = nh_.param<std::string>("status_caption_topic", "/face/status_caption");
        status_max_lines_ = std::max(1, nh_.param<int>("status_max_lines", 80));
        status_color_ = colorNameToCaca(nh_.param<std::string>("status_color", "lightblue"));
        caption_color_ = colorNameToCaca(nh_.param<std::string>("caption_color", "lightmagenta"));
        ...
        sub_backdrop_text_ = nh_.subscribe(backdrop_text_topic_, 10, &FaceNodeCpp::backdropTextCallback, this);
        sub_debug_image_ = nh_.subscribe(debug_image_topic_, 1, &FaceNodeCpp::debugImageCallback, this);
        sub_tts_caption_ = nh_.subscribe("/face/tts_chunk", 10, &FaceNodeCpp::ttsCaptionCallback, this);
        sub_status_caption_ = nh_.subscribe(status_caption_topic_, 10, &FaceNodeCpp::statusCaptionCallback, this);
...
private:
    ...
    ros::Subscriber sub_tts_caption_;
    ros::Subscriber sub_status_caption_;
    ...
    std::string backdrop_text_topic_;
    std::string backdrop_mode_;
    std::string backdrop_direction_;
    std::string backdrop_text_;
    std::deque<std::string> backdrop_lines_;
    ros::Time backdrop_text_start_time_;
    double backdrop_scroll_cps_;
    int backdrop_row_offset_;
    uint8_t backdrop_color_;
    uint8_t backdrop_bg_color_;
    int backdrop_max_lines_;
    ...
    caca_canvas_t* face_canvas_ = nullptr;
    caca_canvas_t* active_text_canvas_ = nullptr;
    caca_dither_t* caca_dither_ = nullptr;
    bool using_caca_display_ = false;

    int canvas_cols_ = 1;
    int canvas_rows_ = 1;
    int face_pane_rows_ = 1;
    int caption_pane_rows_ = 1;
    double caption_pane_fraction_;
    double caption_pane_step_;
    std::string caption_font_;
    std::string status_font_;
    std::string figlet_font_dir_;
    std::string status_caption_topic_;
    int status_max_lines_;
    uint8_t status_color_;
    uint8_t caption_color_;

    std::vector<std::string> current_caption_lines_;
    ros::Time current_caption_start_time_;
    double current_caption_duration_ = 0.0;
    ...
    static std::string sanitizeFigletFont(const std::string& font) {
        std::string clean;
        for (const char ch : font) {
            if (std::isalnum(static_cast<unsigned char>(ch)) || ch == '_' || ch == '-') {
                clean.push_back(ch);
            }
        }
        return clean.empty() ? "term" : clean;
    }
    ...
    static std::string findDefaultFigletFontDir() {
        const std::vector<std::string> candidates = {
            "/home/robot/.local/lib/python3.8/site-packages/pyfiglet/fonts",
            "/usr/local/lib/python3.8/dist-packages/pyfiglet/fonts",
            "/usr/lib/python3/dist-packages/pyfiglet/fonts",
            "/usr/share/figlet"
        };

        for (const std::string& dir : candidates) {
            std::ifstream test((dir + "/standard.flf").c_str());
            if (test.good()) {
                return dir;
            }
        }

        return "/usr/share/figlet";
    }

    static std::vector<std::string> renderFigletLines(
        const std::string& text,
        const std::string& font,
        int width,
        const std::string& font_dir
    ) {
        const int safe_width = std::max(8, width);
        const std::string safe_font = sanitizeFigletFont(font);

        char path_template[] = "/tmp/logos_figlet_input_XXXXXX";
        const int fd = mkstemp(path_template);
        if (fd < 0) {
            return splitLines(text);
        }

        const std::string temp_path(path_template);
        {
            std::ofstream out(temp_path.c_str());
            out << text;
        ...
        std::snprintf(
            cmd,
            sizeof(cmd),
            "figlet -d %s -w %d -f %s < %s 2>/dev/null",
            shellQuote(font_dir).c_str(),
            safe_width,
            safe_font.c_str(),
            shellQuote(temp_path).c_str()
        );
        ...
        std::vector<std::string> lines = splitLines(output.empty() ? text : output);
    ...
    void updateDisplayTimer() {
        ...
        display_timer_ = nh_.createTimer(
            ros::Duration(1.0 / static_cast<double>(display_fps_)),
            &FaceNodeCpp::displayCallback,
            this
        );
    }
    ...
    void initCacaLocked() {
        ...
        face_canvas_ = caca_create_canvas(1, 1);
        if (!face_canvas_) {
            ROS_FATAL("Failed to create face sub-canvas.");
            ros::shutdown();
            return;
        }
    ...
    void shutdownCacaLocked() {
        ...
        if (face_canvas_) {
            caca_free_canvas(face_canvas_);
            face_canvas_ = nullptr;
        }

        active_text_canvas_ = nullptr;
    ...
    void ensureRenderGeometryLocked() {
        ...
        canvas_cols_ = std::max(1, caca_get_canvas_width(caca_canvas_));
        canvas_rows_ = std::max(1, caca_get_canvas_height(caca_canvas_));
        caption_pane_rows_ = std::max(
            1,
            std::min(
                canvas_rows_ - 1,
                static_cast<int>(std::lround(canvas_rows_ * caption_pane_fraction_))
            )
        );
        face_pane_rows_ = std::max(1, canvas_rows_ - caption_pane_rows_);

        if (face_canvas_) {
            caca_set_canvas_size(face_canvas_, canvas_cols_, face_pane_rows_);
        }

        const int target_w = std::max(
            min_render_width_,
            static_cast<int>(std::lround(canvas_cols_ * render_px_per_char_x_))
        );

        const int target_h = std::max(
            min_render_height_,
            static_cast<int>(std::lround(face_pane_rows_ * render_px_per_char_y_))
        );
    ...
    void backdropTextCallback(const std_msgs::String::ConstPtr& msg) {
        std::lock_guard<std::mutex> lock(param_mutex_);
        applyBackdropCommandLocked(msg->data);
        backdrop_text_start_time_ = ros::Time::now();
    }
    ...
    void ttsCaptionCallback(const logos_msgs::SpeechData::ConstPtr& msg) {
        std::lock_guard<std::mutex> lock(param_mutex_);
        const int caption_width = std::max(8, canvas_cols_);
        current_caption_lines_ = renderFigletLines(
            msg->text_snippet,
            caption_font_,
            caption_width,
            figlet_font_dir_
        );
        current_caption_start_time_ = ros::Time::now();
        current_caption_duration_ = std::max(0.05, static_cast<double>(msg->duration));
        caption_active_ = true;
    }

    void statusCaptionCallback(const std_msgs::String::ConstPtr& msg) {
        std::lock_guard<std::mutex> lock(param_mutex_);
        applyStatusCaptionLocked(msg->data);
    }

    void applyStatusCaptionLocked(const std::string& payload) {
        if (payload.empty()) {
            return;
        }

        std::string text = payload;
        std::string font = status_font_;
        ...
        if (payload.front() == '{') {
            try {
                ...
                if (color) {
                    status_color_ = colorNameToCaca(*color);
                }

                font = root.get<std::string>("font", font);
                text = root.get<std::string>("text", "");
            } catch (const std::exception& e) {
                ROS_WARN_STREAM("Failed to parse status caption JSON; using payload as text: " << e.what());
                text = payload;
            }
        }
        ...
        const std::vector<std::string> lines = renderFigletLines(
            text,
            font,
            std::max(8, canvas_cols_),
            figlet_font_dir_
        );
    ...
    void applyBackdropCommandLocked(const std::string& payload) {
        if (payload.empty()) {
            backdrop_text_.clear();
            backdrop_lines_.clear();
            return;
        }

        if (payload.front() != '{') {
            setBackdropTextLocked(payload, true);
            return;
        }
        ...
        try {
            ...
            backdrop_mode_ = toLower(root.get<std::string>("mode", backdrop_mode_));
            backdrop_direction_ = toLower(root.get<std::string>("direction", backdrop_direction_));
            backdrop_scroll_cps_ = std::max(0.0, root.get<double>("speed", backdrop_scroll_cps_));
            backdrop_row_offset_ = std::max(0, root.get<int>("row_offset", backdrop_row_offset_));
            backdrop_max_lines_ = std::max(1, root.get<int>("max_lines", backdrop_max_lines_));

            const boost::optional<std::string> color = root.get_optional<std::string>("color");
            if (color) {
                backdrop_color_ = colorNameToCaca(*color);
            }

            const boost::optional<std::string> bg_color = root.get_optional<std::string>("bg_color");
            if (bg_color) {
                backdrop_bg_color_ = colorNameToCaca(*bg_color);
            }

            const boost::optional<std::string> text = root.get_optional<std::string>("text");
            if (text) {
                const bool append_default = (backdrop_mode_ == "terminal");
                setBackdropTextLocked(*text, root.get<bool>("append", append_default));
            }
        } catch (const std::exception& e) {
            ROS_WARN_STREAM("Failed to parse backdrop JSON; using payload as text: " << e.what());
            setBackdropTextLocked(payload, true);
        }
    }

    void setBackdropTextLocked(const std::string& text, bool append) {
        backdrop_text_ = text;
        ...
        for (const std::string& line : lines) {
            backdrop_lines_.push_back(line);
        }

        while (static_cast<int>(backdrop_lines_.size()) > backdrop_max_lines_) {
            backdrop_lines_.pop_front();
        }
    }
    ...
    void composeDisplayCanvasLocked() {
        if (!caca_canvas_) {
            return;
        }
        ...
        if (face_canvas_) {
            caca_blit(caca_canvas_, 0, 0, face_canvas_, nullptr);
        }

        drawStatusCaptionsLocked();
        drawCaptionPaneLocked();
        drawPaneSeparatorLocked();
    }

    void drawPaneSeparatorLocked() {
        if (!caca_canvas_ || caption_pane_rows_ <= 0 || face_pane_rows_ <= 0) {
            return;
        }

        const int y = std::max(0, std::min(canvas_rows_ - 1, face_pane_rows_));
        caca_set_color_ansi(caca_canvas_, CACA_DARKGRAY, CACA_BLACK);
        for (int x = 0; x < canvas_cols_; ++x) {
            caca_put_char(caca_canvas_, x, y, '-');
        }
    }

    void drawStatusCaptionsLocked() {
        if (!caca_canvas_ || status_lines_.empty() || face_pane_rows_ <= 1) {
            return;
        }

        caca_set_color_ansi(caca_canvas_, status_color_, CACA_BLACK);
        const int available_rows = std::max(1, face_pane_rows_ - 1);
        const int visible_count = std::min(available_rows, static_cast<int>(status_lines_.size()));
        const int first_line = static_cast<int>(status_lines_.size()) - visible_count;
        const int first_y = std::max(0, face_pane_rows_ - visible_count - 1);

        for (int i = 0; i < visible_count; ++i) {
            drawAsciiLineLocked(status_lines_[first_line + i], 0, first_y + i, canvas_cols_);
        }
    }

    void drawCaptionPaneLocked() {
        if (!caca_canvas_ || !caption_active_ || current_caption_lines_.empty() || caption_pane_rows_ <= 0) {
            return;
        }

        const double elapsed = std::max(0.0, (ros::Time::now() - current_caption_start_time_).toSec());
        const double progress = current_caption_duration_ > 0.0
            ? clampDouble(elapsed / current_caption_duration_, 0.0, 1.0)
            : 1.0;
        int revealed = static_cast<int>(std::ceil(progress * current_caption_lines_.size()));
        revealed = std::max(1, std::min(revealed, static_cast<int>(current_caption_lines_.size())));

        caca_set_color_ansi(caca_canvas_, caption_color_, CACA_BLACK);

        const int available_rows = std::max(1, caption_pane_rows_ - 1);
        const int visible_count = std::min(available_rows, revealed);
        const int first_line = std::max(0, revealed - visible_count);
        const int first_y = face_pane_rows_ + 1;

        for (int i = 0; i < visible_count; ++i) {
            drawAsciiLineLocked(current_caption_lines_[first_line + i], 0, first_y + i, canvas_cols_);
        }
    }

    void drawAsciiLineLocked(const std::string& line, int x, int y, int max_width) {
        if (!caca_canvas_ || y < 0 || y >= canvas_rows_) {
            return;
        }

        for (int i = 0; i < max_width && i < static_cast<int>(line.size()); ++i) {
            const unsigned char ch = static_cast<unsigned char>(line[i]);
            if (ch >= 0x20 && ch < 0x7f) {
                caca_put_char(caca_canvas_, x + i, y, ch);
            }
        }
    }
    ...
    void ditherToCanvasLocked(const cv::Mat& img) {
        ...
        caca_clear_canvas(face_canvas_);

        int w = caca_get_canvas_width(face_canvas_);
        int h = caca_get_canvas_height(face_canvas_);
        ...
        active_text_canvas_ = face_canvas_;
        drawTextBackdropLocked(w, h);
        active_text_canvas_ = nullptr;
        caca_dither_bitmap(face_canvas_, 0, 0, w, h, caca_dither_, rgba_.data);
    }
    ...
    void drawTextBackdropLocked(int w, int h) {
        if (backdrop_mode_ == "terminal") {
            drawTerminalBackdropLocked(w, h);
            return;
        }

        drawMarqueeBackdropLocked(w, h);
    }

    void drawMarqueeBackdropLocked(int w, int h) {
        caca_canvas_t* canvas = active_text_canvas_ ? active_text_canvas_ : caca_canvas_;
        if (!canvas) {
            return;
        }

        if (backdrop_text_.empty()) {
            return;
        }

        const std::string pattern = backdrop_text_ + "   ";
        const int pattern_len = static_cast<int>(pattern.size());
        if (pattern_len < 1) {
            return;
        }

        const double elapsed = std::max(0.0, (ros::Time::now() - backdrop_text_start_time_).toSec());
        int scroll = static_cast<int>(std::floor(elapsed * backdrop_scroll_cps_));
        if (backdrop_direction_ == "right") {
            scroll = -scroll;
        }

        caca_set_color_ansi(canvas, backdrop_color_, backdrop_bg_color_);

        for (int y = 0; y < h; ++y) {
            const int row_offset = scroll + y * backdrop_row_offset_;
            for (int x = 0; x < w; ++x) {
                int idx = (x + row_offset) % pattern_len;
                if (idx < 0) {
                    idx += pattern_len;
                }

                const unsigned char ch = static_cast<unsigned char>(pattern[idx]);
                if (ch >= 0x20 && ch < 0x7f) {
                    caca_put_char(canvas, x, y, ch);
                }
            }
        }
    }

    void drawTerminalBackdropLocked(int w, int h) {
        caca_canvas_t* canvas = active_text_canvas_ ? active_text_canvas_ : caca_canvas_;
        if (!canvas) {
            return;
        }

        if (backdrop_lines_.empty()) {
            return;
        }

        caca_set_color_ansi(canvas, backdrop_color_, backdrop_bg_color_);

        const int visible_count = std::min(h, static_cast<int>(backdrop_lines_.size()));
        const int first_line = static_cast<int>(backdrop_lines_.size()) - visible_count;
        const int first_y = (backdrop_direction_ == "down") ? 0 : h - visible_count;
        ...
        for (int i = 0; i < visible_count; ++i) {
            ...
            const std::string& line = backdrop_lines_[first_line + i];
            for (int x = 0; x < w && x < static_cast<int>(line.size()); ++x) {
                const unsigned char ch = static_cast<unsigned char>(line[x]);
                if (ch >= 0x20 && ch < 0x7f) {
                    caca_put_char(canvas, x, y, ch);
                }
            }
    ...
    void handleKeyPress(char key) {
        ...
        if (key == KEY_INCREASE_CAPTION_PANE || key == KEY_INCREASE_CAPTION_PANE_ALT) {
            caption_pane_fraction_ = clampDouble(
                caption_pane_fraction_ + caption_pane_step_,
                0.05,
                0.80
            );
            ensureRenderGeometryLocked();
        ...
        if (key == KEY_DECREASE_CAPTION_PANE) {
            caption_pane_fraction_ = clampDouble(
                caption_pane_fraction_ - caption_pane_step_,
                0.05,
                0.80
