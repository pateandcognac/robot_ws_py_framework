...
import random
import time
import re # For splitting text and emoji
import json # For loading emoji presets
import glob # For finding emoji preset files
...
class FaceAmbienceNode:
    def __init__(self):
        ...
        try:
            ...
            self.min_fps = rospy.get_param('~min_fps', 2) # Get from param or default
            self.def_fps = rospy.get_param('~default_fps',12) # Get from param or default
    ...
    def publish_random_idle_eye_parameters(self):
        ...
        scale_x = random.uniform(0.3, 1)
        scale_y = random.uniform(0.3, 1)
        lid_height = random.uniform(-1, 1)
        lid_angle = random.randint(-35, 35)
    ...
    def _trigger_activity_pause(self, duration: float):
        ...
        with self.timer_lock:
            # Set pause timer to current time + duration, effectively the end time of the pause
    ...
    def handle_tts_chunk_activity(self, msg: SpeechData):
        ...
        pause_extension = rospy.get_param('~tts_pause_extension', 1.0) # e.g., 1 extra second
        effective_pause_duration = msg.duration + pause_extension
    ...
    def manage_fps_reduction_timer(self):
        ...
        with self.timer_lock:
            ...
            if self.fps_reduction_start_time is not None:
                ...
                time_before_next_reduction_step = rospy.get_param('~fps_reduction_step_interval', 60.0)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if perform_idle_animation:
                ...
                if random.random() < 0.9: # Chance to publish mouth wave
                    self.publish_random_idle_sine_wave()
                if random.random() < 0.8: # Chance to publish eye params
