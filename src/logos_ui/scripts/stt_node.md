...
The STT node is Logos's ears. It handles all microphone input in one process: wake word detection, hotword listening, speech-to-text transcription, ambient audio transcription, and background audio classification. Everything stays in one node to avoid ROS serialization overhead on the audio stream.
...
Wake word (`hey-robot`) is **always on** — it doesn't need to be enabled.
...
1. Say **"Hey Robot"** — Porcupine detects the wake word
...
5. Say **"Edit Input"** instead to get a terminal prompt to edit the transcript first
...
Plain string — the name of the detected keyword. One message per detection (not latched, not debounced by default).
...
`hey-robot`, `end-of-line`, and `edit-input` are custom Porcupine models. The rest are Porcupine built-ins that are published when hotword listening is enabled.
...
| `HOTWORD_DEBOUNCE_SEC` | 0.0 | Min seconds between repeated hotword publishes |
| `GATE_AMBIENT_BUILTINS_WITH_VAD` | False | If true, only check built-in hotwords when VAD detects speech |
